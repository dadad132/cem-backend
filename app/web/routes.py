...
async def web_admin_updates(
    ...
    """Admin page to check for and manage system updates"""
    user_id = request.session.get('user_id')
    if not user_id:
        return RedirectResponse('/web/login', status_code=303)
    ...
    # Load version info and check for GitHub updates
    from app.core.version import VERSION, BUILD_DATE
    from app.core.updates import load_update_info, check_github_updates
    
    update_info = load_update_info()
    github_info = check_github_updates()
...
@router.post('/admin/updates/check-github')
async def web_admin_check_github_updates(
    request: Request,
    db: AsyncSession = Depends(get_session),
):
    ...
    if not user or not user.is_admin:
        return RedirectResponse('/web/admin/updates', status_code=303)
    
    from app.core.updates import check_github_updates
    check_github_updates()
    
    return RedirectResponse('/web/admin/updates', status_code=303)


@router.post('/admin/updates/install-github')
async def web_admin_install_github_update(
    request: Request,
    db: AsyncSession = Depends(get_session),
):
    """Install update from GitHub (runs update script in background)"""
    user_id = request.session.get('user_id')
    if not user_id:
        return RedirectResponse('/web/login', status_code=303)
    ...
    from app.core.updates import trigger_github_update
    success = trigger_github_update()
    
    if success:
        # The update script will restart the service, so redirect won't be seen
        return templates.TemplateResponse(
            'admin/update_in_progress.html',
            {
                'request': request,
                'user': user,
                'message': 'Update is installing... The system will restart automatically.'
            }
        )
    else:
        return RedirectResponse('/web/admin/updates?error=update_failed', status_code=303)
